...
#define MEM_BASE_ADDR		XPAR_AXI_7SERIES_DDRX_0_S_AXI_BASEADDR
...
int XAxiDma_SimplePollExample(u16 DeviceId)
{
	...
	for(Index = 0; Index < Tries; Index ++) {
		...
		if (Status != XST_SUCCESS) {
			xil_printf("Failed to setup receive\r\n");
			return XST_FAILURE;
		}
		...
		if (Status != XST_SUCCESS) {
			xil_printf("Failed to setup transmit\r\n");
			return XST_FAILURE;
		}
		xil_printf("Waiting for transmit\r\n");
		while (XAxiDma_Busy(&AxiDma,XAXIDMA_DMA_TO_DEVICE)) {
		}
		xil_printf("Waiting for receive\r\n");
